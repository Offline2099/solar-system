...
import { objectData } from '../../utils/celestial-object';
import './CelestialObjectDataContainer.scss';

const CelestialObjectData = (data: CelestialEntityData) => {
  const sections = useMemo(() => objectData(data), [data]);
  const [collapsedSections, setCollapsedState] = useState(new Set(sections.keys()));
  ...
  return (
    <div className={`data-container ${ENTITY_TYPE_NAME[data.type]}`}>
      {sections.map((section, sectionIndex) => (
        <div
          key={sectionIndex}
          className={`data-section${collapsedSections.has(sectionIndex) ? ' collapsed' : ''}`}
        >
